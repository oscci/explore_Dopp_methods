...
mypairs<-matrix(c(1,2,1,3,3,4,2,4),nrow=4,byrow=TRUE)
  for (i in 1:4){
    ...
    plot(fordemo[,a],fordemo[,b],xlab=colnames(fordemo)[a],ylab=colnames(fordemo)[b],xlim=c(-3,5),ylim=c(-3,5),cex=.75)
...
if(i==2){myylab<-'Task B.1'}
    plot(fordemo[,a],fordemo[,b],xlim=c(-3,5),ylim=c(-3,5),pch=16,col='blue',frame.plot=FALSE,cex=.7,xlab="",ylab="")
    title(ylab=myylab,line=2)
       title(xlab=myxlab,line=2)
   abline(h=mean(fordemo[,a]))
   abline(v=mean(fordemo[,b]))
   rbit<-paste('r = ',round(cor(fordemo[,a],fordemo[b]),2))
   text(-2,3.5,rbit)
  }
