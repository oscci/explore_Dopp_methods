...
if(i==2){myylab<-'Task B.1'}
    plot(fordemo[1:150,a],fordemo[1:150,b],xlim=c(-3,5),ylim=c(-3,5),pch=16,col='blue',frame.plot=FALSE,cex.axis=.7,xlab="",ylab="")
    ...
    if (k==1){
    title(xlab=expression('LI'['ij']*' = a + e'['ij']),line=2)}
        if (k==3){
    title(xlab=expression('LI'['ij']*' = a + t'['i']*' + e'['ij']),line=2)}
            if (k==4){
    title(xlab=expression('LI'['ij']*' = a + t'['i']*' + p'['j']*' + e'['ij']),line=2)}
               if (k==5){
    title(xlab=expression('LI'['ij']*' = a + t'['i']*' + p'['j']*' + x'['ij']*' + e'['ij']),line=2)}

   # title(ylab=myylab,line=2)
      # title(xlab=myxlab,line=2)
   abline(h=mean(fordemo[,a]),lty=2)
   abline(v=mean(fordemo[,b]),lty=2)
